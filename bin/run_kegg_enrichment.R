...
## =============================================================================
## Script:  run_kegg_enrichment.R
## Purpose: Perform KEGG pathway enrichment analysis for DE genes, WGCNA modules, MaSigPro clusters, DIU, and DEU genes
## Usage:   Rscript run_kegg_enrichment.R <degFile> <pvalThreshold> <lfcThreshold> <speciesName> <wgcnaList> <masigproList> <diuGenes> <deuGenes> <ntopProcesses>
## Example: Rscript run_kegg_enrichment.R deg.rds 0.05 1.0 human wgcna.rds masigpro.rds diu.rds deu.rds 20
## =============================================================================
...
if (length(args) < 9) {
    stop(
        "Usage:\n",
        "run_kegg_enrichment.R <degFile> <pvalThreshold> <lfcThreshold> <speciesName> <wgcnaList> <masigproList> <diuGenes> <deuGenes> <ntopProcesses>",
        call. = FALSE
    )
}
...
## =============================================================================
## Species Database with KEGG organism codes
## =============================================================================

speciesDb <- list(
    human = list(orgdb = "org.Hs.eg.db", biomart = "hsapiens_gene_ensembl", kegg = "hsa"),
    mouse = list(orgdb = "org.Mm.eg.db", biomart = "mmusculus_gene_ensembl", kegg = "mmu"),
    rat   = list(orgdb = "org.Rn.eg.db", biomart = "rnorvegicus_gene_ensembl", kegg = "rno"),
    yeast = list(orgdb = "org.Sc.sgd.db", biomart = "scerevisiae_gene_ensembl", kegg = "sce"),
    fruitfly = list(orgdb = "org.Dm.eg.db", biomart = "dmelanogaster_gene_ensembl", kegg = "dme"),
    zebrafish = list(orgdb = "org.Dr.eg.db", biomart = "drerio_gene_ensembl", kegg = "dre"),
    worm = list(orgdb = "org.Ce.eg.db", biomart = "celegans_gene_ensembl", kegg = "cel"),
    arabidopsis = list(orgdb = "org.At.tair.db", biomart = "athaliana_eg_gene", kegg = "ath"),
    chicken = list(orgdb = "org.Gg.eg.db", biomart = "ggallus_gene_ensembl", kegg = "gga"),
    cow = list(orgdb = "org.Bt.eg.db", biomart = "btaurus_gene_ensembl", kegg = "bta"),
    pig = list(orgdb = "org.Ss.eg.db", biomart = "sscrofa_gene_ensembl", kegg = "ssc"),
    dog = list(orgdb = "org.Cf.eg.db", biomart = "clfamiliaris_gene_ensembl", kegg = "cfa"),
    monkey = list(orgdb = "org.Mmu.eg.db", biomart = "mmulatta_gene_ensembl", kegg = "mcc")
)
...
runKeggEnrichment <- function(symbols, label) {
    ...
    createDirsFromLabel <- function(label) {
        ...
        if (length(parts) == 1) {
            dir.create(file.path("KEGG_results", label), showWarnings = FALSE, recursive = TRUE)
            baseDir <- file.path("KEGG_results", label)
            plotDir <- file.path(baseDir, "plots")
            csvDir <- file.path(baseDir, "csv")
        } else if (length(parts) == 2) {
            dir.create(file.path("KEGG_results", parts[1]), showWarnings = FALSE, recursive = TRUE)
            dir.create(file.path("KEGG_results", parts[1], parts[2]), showWarnings = FALSE, recursive = TRUE)
            baseDir <- file.path("KEGG_results", parts[1], parts[2])
            plotDir <- file.path(baseDir, "plots")
            csvDir <- file.path(baseDir, "csv")
        } else {
            dirPath <- file.path("KEGG_results", paste(parts[-length(parts)], collapse = "/"))
    ...
    if (is.null(symbols) || length(symbols) < 5) {
        writeReasonPng(
            file.path(plotDir, paste0(label, "_KEGG.png")),
            paste0("Not enough genes for KEGG enrichment: ", label)
        )
        return()
    }

    ## Check if KEGG organism code is available
    if (is.null(speciesInfo$kegg)) {
        writeReasonPng(
            file.path(plotDir, paste0(label, "_KEGG.png")),
            paste0("No KEGG organism code available for ", speciesName)
        )
        return()
    }
    ...
    keggResult <- tryCatch({
        enrichKEGG(
            gene = symbols,
            keyType = "ncbi-geneid",
            organism = speciesInfo$kegg,
            pvalueCutoff = pvalThreshold
        )
    }, error = function(e) {
        cat("Error in enrichKEGG for", label, ":", e$message, "\n")
        NULL
    })

    if (!is.null(keggResult) && nrow(keggResult) > 0) {
        tryCatch({
            ggsave(
                file.path(plotDir, paste0(label, "_KEGG.pdf")),
                dotplot(keggResult, showCategory = ntopProcesses) +
                    ggtitle(paste("KEGG Pathway Enrichment -", label)),
                width = 12, height = 10
            )
            write.csv(keggResult@result,
                    file.path(csvDir, paste0(label, "_KEGG.csv")),
                    row.names = FALSE)
            cat("Successfully generated KEGG enrichment for", label, "\n")
        }, error = function(e) {
            cat("Error saving results for", label, ":", e$message, "\n")
        })
    } else {
        writeReasonPng(
            file.path(plotDir, paste0(label, "_KEGG.png")),
            paste0("No significant KEGG pathways for ", label)
        )
...
if (degFile == "null") {
    writeReasonPng("KEGG_results/KEGG_DE.png", "No DE genes detected")
} else {
    ...
    runKeggEnrichment(upSymbols, "DEGs_Up")
    runKeggEnrichment(downSymbols, "DEGs_Down")
}

## WGCNA modules
if (wgcnaListFile == "null") {
    writeReasonPng("KEGG_results/KEGG_WGCNA.png", "No genes detected from WGCNA modules")
} else {
    ...
    for (traitName in names(wgcnaData)) {
        ...
        for (moduleName in names(traitModules)) {
            ...
            runKeggEnrichment(symbolsConverted, label)
        }
    }
}
...
if (masigproListFile == "null") {
    writeReasonPng("KEGG_results/KEGG_masigpro.png", "No genes detected from MaSigpro clusters")
} else {
...
if (diuGenesFile == "null") {
    cat("No DIU genes file provided, skipping DIU KEGG enrichment\n")
} else {
    diuGenes <- readRDS(diuGenesFile)
    if (is.null(diuGenes) || length(diuGenes) == 0) {
        writeReasonPng("KEGG_results/KEGG_DIU.png", "No genes detected from DIU genes")
        cat("No DIU genes found\n")
    } else {
        symbolsConverted <- convertSymbols(diuGenes)
        runKeggEnrichment(symbolsConverted, "DIU")
        cat("DIU KEGG enrichment completed\n")
    }
}

## DEU genes
if (deuGenesFile == "null") {
    cat("No DEU genes file provided, skipping DEU KEGG enrichment\n")
} else {
    deuGenes <- readRDS(deuGenesFile)
    if (is.null(deuGenes) || length(deuGenes) == 0) {
        writeReasonPng("KEGG_results/KEGG_DEU.png", "No genes detected from DEU genes")
        cat("No DEU genes found\n")
    } else {
        symbolsConverted <- convertSymbols(deuGenes)
        runKeggEnrichment(symbolsConverted, "DEU")
        cat("DEU KEGG enrichment completed\n")
    }
}
