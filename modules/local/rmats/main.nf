process RMATS {
    ...
    tuple val(meta4), path(rmats_prep_tmp_dir)
    tuple val(meta5), val(avg_length)
    val mode

    output:
    tuple val(meta), path("rmats_post_output"), emit: post_output, optional: true
    tuple val(meta), path("rmats_prep_tmp"), emit: prep_tmp, optional: true
    tuple val("${task.process}"), val('rmats'), eval("rmats.py --version | sed 's/v//'"), topic: versions, emit: versions_rmats
    ...
    def read_type = meta.single_end ? "-t single" : "-t paired"
    def strand_flag = ""
    ...
    stub:
    def prefix = mode == 'post' ? 'rmats_post_output' : 'rmats_prep_tmp'
    if (mode == 'prep') {
        """
